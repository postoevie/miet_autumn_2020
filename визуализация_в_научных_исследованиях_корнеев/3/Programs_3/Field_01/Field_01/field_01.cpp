...
void arrowVector(HDC hdc, int x1, int y1, int x2, int y2, COLORREF rgb)
{
	...
	x0 = int(0.85*(x2 - x1)) + x1;
	y0 = int(0.85*(y2 - y1)) + y1;
	x3 = -int(0.07*(y2 - y1)) + x0;
	y3 = int(0.07*(x2 - x1)) + y0;
	x4 = int(0.07*(y2 - y1)) + x0;
	y4 = -int(0.07*(x2 - x1)) + y0;
...
//рисуем 16 линий поля красным цветом
	...
	for(int i=0; i<16; i++)
	{
		...
		xt1 = PointB[i].x; yt1 = PointB[i].y; zt1 = PointB[i].z;
		xt2 = PointB[i].x; yt2 = PointB[i].y; zt2 = PointB[i].z + 0.7;
		...
		arrowVector(hdc,x1,y1,x2,y2, RGB(155,0,255));
	}
